...
template <typename Key>
struct FollyHashFunctionType
{
    using Hash = folly::Hash;
    static constexpr std::string_view description = "folly::Hash";
};
...
void dispatchHashFunctionType(std::string_view hash_function_type, Callback && callback)
{
    ...
    else if (hash_function_type == "folly_hash")
        callback(FollyHashFunctionType<Key>());
